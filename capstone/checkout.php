...
if(isset($_POST['order_btn'])){
   ...
   if($_POST['method'] === 'GCash'){
    ...
    <div class='overlaylay' id='overlaylay'></div> 
      ...
      <h1>E-Receipt of Sale <span>Yantinju Shop</span></h1>
      ...
      <h2 style='text-align: center;'>Customer Name: <span>$lname, $fname</span></h2>
      ...
      while ($product_item2 = mysqli_fetch_assoc($cart_query2)) {
          ...
          $product_name3 = $product_item2['title'];   
  ...
  <h3 style='text-transform: lowercase; font-size:1.3rem;'>This Order is currently being processed. Your items will be shipped once we verify your payment proof.</h3>
   ...
   while($product_item = mysqli_fetch_assoc($cart_query)){
    ...
    if(mysqli_num_rows($delete_query) > 0){
        while($product_item = mysqli_fetch_assoc($delete_query)){
            $product_name2 = $product_item['title'];
            $product_size = $product_item['size']; 
            $product_id = $product_item['id']; 
            $no = $product_item['quantity']; 
            
            
            mysqli_query($conn, "DELETE FROM cart WHERE title='$product_name2' AND phone={$_SESSION['phone']}");
    ...
    $detail_query = mysqli_query($conn, "INSERT INTO `order`(id, customer_id, order_id, fname, lname, number, email, method, address, province, city, brgy, pin_code, total_products, total_price, tax, status, DateTime) VALUES('', '$customer_id', '$order_id', '$fname', '$lname', '$number','$email','$method','$address','$province','$city','$brgy','$pin_code','$total_product','$final_amount', '$with_vat', 'Under Review Order', '$DateTime')");
...
echo"<!-- Container for the image -->
<div class='img-container'>
    <img src='images/gcash-payment2.jpg' alt='GCash Payment'>
    <form action='' method='post' enctype='multipart/form-data'>
        <input type='file' name='proof_photo' accept='image/*' required>
        <input type='submit' class='upload_proof' value='Upload Proof' name='upload_proof'>
        <input type='hidden' name='order_id' value='<?php echo $order_id; ?>'>
    </form>
        <div class='cancel_order'><a href='account.php?my_orders&confirm=logged_in'>Later </a></div>
</div>";

echo "<script>showReceipt();</script>";


   } elseif ($_POST['method'] === 'Cash on Delivery') {
      echo "
...
if(isset($_POST['upload_proof'])){
    ...
    // Retrieve the uploaded proof photo
    $proofpic = $_POST['proof_photo'];

    // Update the user's address details
    $detail_address = mysqli_query($conn, "UPDATE `order` SET gcash_proof = '$proofpic' WHERE order_id='$order_id'");
        ...
        <!--    end of navbar    -->
            ...
            <div class="display-order">
                ...
                <div class="flex">
                    ...
                    <div class="inputBox">
                        <select type="text" name="method" id="paymentMethod" required>
            ...
            function showPaymentDetails() {
                document.getElementById('payment-details').style.display = 'block'; // Show the payment details
                document.getElementById('receipt-container').style.display = 'none'; // Hide the receipt
                document.getElementById('overlaylay').classList.add('show-overlaylay');
            }

            function submitPaymentProof() {
                var paymentProofImage = document.getElementById('payment-proof-image').files[0];
                // Submit payment proof to server using AJAX
                // Example:
                var formData = new FormData();
                formData.append('payment_proof', paymentProofImage);
                // Make AJAX request to submit payment proof
                $.ajax({
                    url: 'submit_proof.php',
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
                    success: function(response) {
                        ...
                        // Optionally, you can redirect the user to a different page or perform any other action
                    },
