...
// Fetch total sold products for the selected month
$sql2 = "SELECT title, count FROM `sold` WHERE DATE_FORMAT(`DateTime`, '%Y-%m') = '$month_of'";
$result2 = mysqli_query($conn, $sql2);

// Initialize an array to store product quantities
$productQuantities = array();

// Variable to store the highest sold count
$highestSoldCount = 0;

// Loop through each sold product
while ($row_pro2 = mysqli_fetch_assoc($result2)) {
    $productName = $row_pro2['title'];
    $quantity = $row_pro2['count'];
    ...
    // Update the highest sold count
    $highestSoldCount = max($highestSoldCount, $quantity);
}
                        ...
                        <div class="panel-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover table-striped">
                                    ...
                                    <tbody>
                                    <?php
                                    foreach ($products_sold as $index => $product) {
                                        $highlight = '';
                                        if ($product['total_sold_products'] == $highestSoldCount && $highestSoldCount != 0) {
                                    ...
                                    $total_amount = array_sum(array_column($products_sold, 'sales_per_product'));
                                    ?>
