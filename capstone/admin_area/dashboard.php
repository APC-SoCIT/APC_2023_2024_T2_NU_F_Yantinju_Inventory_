        ...
        <main>
            ...
            <div class="analytics">
                ...
                <?php
                    ...
                    $count_sold = "SELECT title, count FROM sold";
                    ...
                    $run_query01 = mysqli_query($conn, $count_sold);
                        ...
                        $result01 = mysqli_fetch_assoc($run_query01);
                        ...
                        $product_count2 = $result01['count'];
                        $product_title2 = $result01['title'];
                        $product_rate = $result5['stars'] / $result52['stars'] ;

                            $count_sold3 = "SELECT id FROM paninda WHERE title = '$product_title2'";
                            $run_query33 = mysqli_query($conn, $count_sold3);
                            $result33 = mysqli_fetch_assoc($run_query33);
                            $top_product3 = $result33['id'];

                    // Get the current month
                    $currentMonth = date('Y-m');


                    // Select the product with the highest sold count
                    $count_sold_query = "SELECT title FROM sold ORDER BY count DESC LIMIT 1";
                    $run_count_sold_query = mysqli_query($conn, $count_sold_query);
                    if ($run_count_sold_query) {
                        $result_sold = mysqli_fetch_assoc($run_count_sold_query);
                        $top_product_id = $result_sold['title'];

                        // Retrieve the product title using the top product ID
                        $get_product_title_query = "SELECT id FROM paninda WHERE title = '$top_product_id'";
                        $run_product_title_query = mysqli_query($conn, $get_product_title_query);
                        if ($run_product_title_query) {
                            $result_title = mysqli_fetch_assoc($run_product_title_query);
                            $top_product_title = $result_title['id'];
                        } else {
                            $top_product_title = "No product found";
                        }
                    } else {
                        $top_product_title = "No product found";
                    }
