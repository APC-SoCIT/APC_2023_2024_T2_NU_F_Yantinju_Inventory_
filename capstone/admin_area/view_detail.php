...
?>
                    ...
                    <div class="flex justify-center flex-col md:flex-row flex-col items-stretch w-full space-y-4 md:space-y-0 md:space-x-6 xl:space-x-8">
                        ...
                        <div class="flex justify-between items-start w-full">
                            ...
                            <?php if ($status !== 'Under Review Order'){ ?>
                        ...
                        <?php if ($status === 'Under Review Order'){?>
                            ...
                            <div class="flex justify-center items-center w-full mt-4">
                                <button id="toggleImageButton" type="button" class="text-base dark:text-white leading-4 text-gray-800" style="font-size: 12px; margin-top: -4%;">Toggle Payment Image</button>
                            </div>
                            <div class="hidden" id="imageContainer">
                                <img src="images/payments/<?php echo $picpi ?>" alt="None" id="displayedImage" style="max-width: 200px;">
                            </div>
                    ...
                    <div class="flex flex-col md:flex-row xl:flex-col justify-start items-stretch h-full w-full md:space-x-6 lg:space-x-8 xl:space-x-0">
                        ...
                        ?>
                            ...
                            <p class="text-sm dark:text-gray-300 leading-5 text-gray-600">Thru <?php echo $method; ?></p>
                            </div>
                        </div>
...
if (isset($_POST['change'])) {
    ...
    if ($product_stats == "Product Delivered") {
        $sales_info = mysqli_query($conn, "SELECT * FROM `complete_order` WHERE order_id = '$order_id' AND status='Product Delivered'");
        $row_sales = mysqli_fetch_array($sales_info);

        $get_pro23 = "SELECT * FROM `order` WHERE order_id='$view_id'";
        $run_pro23 = mysqli_query($conn, $get_pro23);

        $row_pro23 = mysqli_fetch_array($run_pro23);
        $products23 = explode(',', $row_pro23['total_products']);
        ...
        foreach ($products23 as $product) {
            ...
            if (preg_match('/^(.*?) \(size (\d+)\) \((\d+)\)$/', trim($product, " \t\n\r\0\x0B("), $matches)) {
                ...
                // Check if the product already exists in the sold table
                $query = "SELECT * FROM `sold` WHERE title = '$item_name'";
                $result = mysqli_query($conn, $query);
        
                if ($result && mysqli_num_rows($result) > 0) {
                    // If the product exists, update the quantity
                    $row = mysqli_fetch_assoc($result);
                    $existing_quantity = (int)$row['count'];
                    $new_quantity = $existing_quantity + $quantity;
        
                    // Update the quantity in the sold table
                    $updateQuery = "UPDATE `sold` SET count = $new_quantity WHERE title = '$item_name'";
                    mysqli_query($conn, $updateQuery);
                } else {
                    // If the product does not exist, insert a new record
                    $currentDateTime = date('Y-m-d');
                    mysqli_query($conn, "INSERT INTO `sold` (id, title, count, DateTime) VALUES ('', '$item_name', $quantity, '$currentDateTime')");
                }
            }
        }
