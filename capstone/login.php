	...
	if (empty($email)) {
		...
		if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['email'] === $email && $row['password'] === $pass && $row['account_activation_hash'] === Null) {
				if ($row['role'] === 'user'){
					if ($row['status'] === 'Activate'){
						...
						header("Location: index.php?confirm=logged_in");
						
						exit();
					} else {
						header("Location: login2.php?error=Invalid Account. You have been disabled");
						exit();
	
					} 
				} else {
					header("Location: login2.php?error=Incorrect Email or Password. Please try again");
					exit();

				} 

            }else{
				header("Location: login2.php?error=Activate your Account before you log in w");
		        exit();	
			}
		}else {
            ...
            if ($row2['email'] === $email && $row2['password'] === $pass && $row2['account_activation_hash'] === Null) {
				...
				header("Location: login2.php?error=Activate your Account before you log in");
		        exit();	
			}
		}
	}
