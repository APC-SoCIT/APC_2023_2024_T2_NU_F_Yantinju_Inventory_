...
if(isset($_GET['products'])) {
   ...
   if($addcart) {
      ...
      if(mysqli_num_rows($addcart) > 0) {
         ...
         $updated = "UPDATE `cart` SET quantity = quantity + 1 WHERE phone = '$phone' AND title = '$title'";
         $updated_product = mysqli_query($conn, $updated);

         if ($updated_product) {
            // Product added successfully
            header("Location: products.php?success=2&product=" . urlencode($productName));
            exit();
        } else {
            // Error adding product
            header("Location: products.php?success=0&product=" . urlencode($productName));
            exit();
        }
      } else {
         // Fetch product details
         ...
         // Insert the product into the cart
         $inserted = "INSERT INTO `cart` (id, phone, title, price, img1, size, quantity) VALUES('', '$phone', '$title', '$price', '$img1', '$sizenumber', '1')";
         $insert_product = mysqli_query($conn, $inserted);

         if ($insert_product) {
            // Product added successfully
            header("Location: products.php?success=1&product=" . urlencode($productName));
            exit();
        } else {
            // Error adding product
            header("Location: products.php?success=0&product=" . urlencode($productName));
